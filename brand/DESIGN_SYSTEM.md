...
`BrandSignature` obedece à distribuição responsiva definida em `SIGNATURE.md`. O header e o footer do site usam `wordmarkOnly`; avatar, favicon, thumbnails, Stories/Reels e brand bug usam `symbol`; o Open Graph principal usa `descriptor`; banners de canal e lower third usam `descriptorWordmark`, que dispensa o símbolo onde o avatar do canal ou o brand bug já apresentam a marca no mesmo campo de visão; `institutional` permanece restrito ao Brandbook e a superfícies institucionais formais.
